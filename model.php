<?php
    ...
    $sql = "SELECT * FROM `todo` WHERE tdNum=$id;";
    ...
    <?php
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($title) || empty($descr)) {
                ...
                $sql = "UPDATE `todo` SET tdNum=$id, title='$title', 
                des='$descr' WHERE tdNum=$id;";
                $result = mysqli_query($con, $sql);
    
                header("Location: ToDoList.php?UpdateTask=Success");
            }
    ...
    <div class="model-bg">
        <div class="model">
            ...
            <form action="<?php echo htmlentities($_SERVER['PHP_SELF']).'?updateTask='.$id; ?>" method="POST">
                ...
                <a id='model-close' href="ToDoList.php">X</a>
            </form>
        </div>
    </div>
</body>
