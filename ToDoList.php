    ...
    <?php
        if (isset($_POST['submit'])) {
            ...
            if (empty($title) || empty($des)) {
                header("Location: ToDoList.php?form=empty");
                exit();
            }else {
                ...
                $sql = "INSERT INTO `todo` (`title`, `des`, `relatedStudent`) VALUES ('$title', '$des', '$relatedStd');";
                $result = mysqli_query($con, $sql);
                header("Location: ToDoList.php?form=success");
            }
        ...
        <div id="table-box">
            <table>
                ...
                <?php
                    ...
                    $sql = "SELECT * FROM `todo` WHERE relatedStudent='$relatedStd'";
                    $result = mysqli_query($con, $sql);
                    $count = 1;
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo '<tr>
                            ...
                            <td>' . $row['des'] . '</td>
                            <td>
                                <button class="btn"><a href="model.php?updateTask=' . $num . '">Update</a></button>
                                <button class="btn"><a href="php_Operation/delete_task.php?deleteTask=' . $num . '">Delete</a></button>
                            </td>
                            </tr>';
                            $count++;
                    };
